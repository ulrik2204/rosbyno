...
void main() {
    mouseX = (u_mousePosition.x - 0.5);
    mouseY = (u_mousePosition.y - 0.5);
    distortion = pnoise(normal + mouseY / 3. + mouseX / 3., vec3(10.));

    vUv = uv;
    vec3 pos = position;

    xDistortion = sin(u_time + uv.y * 20.);
    zDistortion = cos(u_time + uv.y * 20.);

    pos.x += xDistortion * 10.;
    pos.z -= zDistortion * 5.;
    // pos.xz += distortion * 15.;
    // pos.y += distortion * 15.;
    // pos.x += distortion * 8.;

    color1 = pnoise(normal, vec3(3.)) * 1.1;
    color2 = pnoise(normal + 0.29, vec3(10.)) * 1.;
    color3 = pnoise(normal + 0.4, vec3(1.)) * 1.4;
    color4 = pnoise(normal + 0.4, vec3(1.)) * 1.4;
